...
from openai.types.chat.chat_completion_chunk import ChoiceDeltaToolCallFunction, ChoiceDeltaFunctionCall, \
    ChoiceDeltaToolCall
...
try:
    ...
    ERROR = ImportError("Please install diskcache to use autogen.OpenAIWrapper.")
...
class OpenAIWrapperFactory:
    ...
    def get_client(cls, **config):
        ...
        if config_key not in cls._clients:
            cls._clients[config_key] = OpenAIWrapper(**config)
