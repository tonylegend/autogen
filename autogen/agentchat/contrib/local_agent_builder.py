...
from aix.utils.misc import get_gpt_assistants_brief, normalize_agent_name
...
class AgentBuilder:
    ...
    # The less of the agent names the better. Create a new agent name only when it is required and not in 'EXISTING_POSITIONS'.
...
output after executing the code) and provide a corrected answer or code.
    ...
    def __init__(
        ...
        self.building_task: str | None = None
        ...
        self.agents = []
        self.existing_positions = get_gpt_assistants_brief()
    ...
    def _create_agent(
        ...
        """
        config_list = get_config_list(self.config_path, filter_dict={"model": [model_name_or_hf_repo]})
        if len(config_list) == 0:
            raise RuntimeError(
                f"Fail to initialize agent {agent_name}: {model_name_or_hf_repo} does not exist in {self.config_path}.\n"
                f'If you would like to change this model, please specify the "agent_model" in the constructor.\n'
                f"If you load configs from json, make sure the model in agent_configs is in the {self.config_path}."
            )
    ...
    def build(
        ...
        **kwargs,
        ...
        config_list = get_config_list(self.config_path, filter_dict={"model": [self.builder_model]})
        if len(config_list) == 0:
            raise RuntimeError(
                ...
                messages=[
                    {
                        ...
                        "content": self.AGENT_NAME_PROMPT.format(
                            ...
                            existing_positions=list(self.existing_positions.values())),
                    }
                ]
            )
            .choices[0]
        ...
        # agent_name_list = [re.sub(r"[{}]".format("".join({"\\\\",})), "", agent_name.strip()).replace(" ", "_").replace("*", "_") for agent_name in resp_agent_name.split(",")]
        agent_name_list = [normalize_agent_name(agent_name) for agent_name in resp_agent_name.split(",")]
        existing_agent_names = [v['name'] for v in self.existing_positions.values()]
        print(f"{agent_name_list} are generated. Existing agent name: {existing_agent_names}.")

        for name in agent_name_list:
            print(f"Preparing configuration for {name}...")
            if name in existing_agent_names:
                assistant_id = next((k for k, v in self.existing_positions.items() if v['name'] == name), None)
                model = self.agent_model
                llm_config = (default_llm_config.copy() or {}) | {'assistant_id': assistant_id} if assistant_id \
                    else default_llm_config
                configs = {
                    "agent_name": name,
                    "model_name_or_hf_repo": model,
                    "system_message": self.AGENT_SYS_MSG_PROMPT.format(
                        task=building_task,
                        position=name,
                ...
                if next((v['coding'] for v in self.existing_positions.values() if v['name'] == name), False):
                    coding = True
            else:
                ...
                configs = {
                    ...
                    "system_message": resp_agent_sys_msg,
                    "description": resp_agent_description,
                    "llm_config": default_llm_config.copy(),
                    "use_oai_assistant": use_oai_assistant,
                }
            agent_configs.append(configs)
        ...
        **kwargs,
        ...
        for name, sys_msg, description in list(zip(agent_name_list, agent_sys_msg_list, agent_profile_list)):
            agent_configs.append(
                {"agent_name": name, "model_name_or_hf_repo": self.agent_model, "system_message": sys_msg,
                 "description": description, "llm_config": default_llm_config.copy(),
                 "use_oai_assistant": use_oai_assistant}
            )
    ...
    def select_from_existing_gpts(
        ...
        print("==> Looking for suitable agents in existing agents...")
        if embedding_model is not None:
            chroma_client = chromadb.Client()
            collection = chroma_client.create_collection(
                name="agent_list",
            ...
            collection.add(
                documents=[v["description"] for v in self.existing_positions.values()],
                metadatas=[{"source": "agent_description"} for _ in range(len(self.existing_positions))],
                ids=[f"agent_{i}" for i in range(len(self.existing_positions))],
            )
            agent_profile_list = collection.query(query_texts=[building_task], n_results=self.max_agents)["documents"][
                0
            ]
            ...
            agent_name_list = []
            for profile in agent_profile_list:
                for agent in self.existing_positions.values():
                    if agent["description"] == profile:
                        agent_name_list.append(agent["name"])
                        break
            chroma_client.delete_collection(collection.name)
        else:
            agent_profiles = [
                f"No.{i + 1} AGENT's NAME: {agent['name']}\nNo.{i + 1} AGENT's PROFILE: {agent['description']}\n\n"
                for i, agent in enumerate(list(self.existing_positions.values()))
            ]
            resp_agent_name = (
                build_manager.create(
                    messages=[
    ...
    def _build_agents(
        ...
        for config in agent_configs:
            print(f"Creating agent {config['agent_name']} with backbone {config['model_name_or_hf_repo']}...")
            self._create_agent(**config, **kwargs)
        ...
        if coding is True:
            ...
            self.agents = [
                autogen.UserProxyAgent(
                    ...
                    llm_config={"config_list": config_list, "timeout": 120, "cache_seed": 42},
                )
            ] + self.agents

        return self.agents, self.cached_configs.copy()
